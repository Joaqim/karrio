...
> **v1.6 revision note:** D9's "rating performs no carrier call" property is **deliberately relaxed** by D11 following ingestion of the Transit Time V2 spec. `rate()` now always issues one `GET /rest/transport/v2/transittime/addresstoaddress` call per request to enrich each rate with an accurate `transit_days` and `meta.estimated_delivery`, and to drop services PostNord reports as not bookable (`isBookable=false`). Price resolution is unchanged — it still uses the universal `RatingMixinProxy` over the server-side `RateSheet` (D9); the transit lookup only enriches and filters. The call is guarded: on any transit failure (network, non-200, unparseable body) rating degrades gracefully — static `transit_days` are kept, no serviceability filtering is applied, and a single `transit_time_unavailable` warning is surfaced — so price rating never hard-fails. This trades D9's offline/no-dependency property for ETA accuracy and live serviceability, a trade-off accepted explicitly.
...
| D11 | Transit times in rating (relaxes D9) | Always call Transit Time V2 in `rate()`; graceful degrade on failure | PostNord's Transit Time V2 (`GET /rest/transport/v2/transittime/addresstoaddress`) yields accurate `transit_days`, estimated delivery, and `isBookable` serviceability. `rate()` issues one call per request, enriching the static-priced rates and dropping unbookable services; price resolution stays static (D9). A guarded call degrades to static transit days with a `transit_time_unavailable` warning on any failure, so price rating never hard-fails. Accepts a carrier-call dependency in the rate path in exchange for ETA accuracy and serviceability. | 2026-06-24 |
