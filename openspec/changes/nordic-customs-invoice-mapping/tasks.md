...
- [ ] 2.2 Correct `BuyerType.postalCode` and `reasonForExportation` to strings in the PostNord schema sample (`schemas/shipment_request.json`) and regenerate with `./bin/run-generate-on modules/connectors/postnord`, and verify a unit test asserting a Norwegian postal code `0154` survives serialization in the customs invoice
