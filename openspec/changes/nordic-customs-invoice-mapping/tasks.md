...
- [ ] 2.7 Multiply per-unit commodity value and weight by quantity for CN22 lines, customs invoice lines, and both declaration totals (user confirmed 2026-09-25 that production CN22s show a third of the expected value and weight for quantity 3), and verify unit tests asserting line and total values for a quantity-3 commodity on both paths
...
- [ ] 5.6 Convert customs commodity net weight to kilograms regardless of the commodity weight unit (per-unit versus line-total semantics of `customsValue` and `netWeight` stay unchanged pending DHL confirmation), and verify unit tests asserting a pound- and an ounce-weighted commodity yield kilogram `netWeight`
