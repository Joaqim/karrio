...
- `plugins/shipment-advisors`: how plugins declare shipment advisors, when the SDK runs them during rating and shipment creation, and the non-blocking, failure-isolated contract of their messages.
...
- `modules/sdk/karrio/core/metadata.py` (new `PluginMetadata` field), `modules/sdk/karrio/references.py` (collection), `modules/sdk/karrio/api/interface.py` (invocation in rating and shipment creation), and SDK tests; no generated files, connectors, or server code change.
