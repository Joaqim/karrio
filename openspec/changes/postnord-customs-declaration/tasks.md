...
- [ ] 4.3 Verify the 13-line guard applies to the proxy path with a unit test asserting the field error and no HTTP call for an over-limit declaration, reusing one shared guard helper beside `CUSTOMS_DECLARATION_MAX_LINES` rather than duplicating the booking path's check (design D5)
