...
- [x] 5.2 Build the documents-module server surface for stamping in `modules/documents/karrio/server/documents/`; verify against a server fixture. PRD at `PRDs/DOCUMENT_STAMPING_SERVER_ENDPOINT.md`. Implemented as REST `POST v1/documents/stamp` (`views/stamping.py`, `DocumentStamper`) wrapping `lib.stamp_document` — mirroring `POST /documents/generate`, not a new weasyprint pipeline. Decisions: REST-only (Q1/D1), base64-in only with no stored-document/org-scoped variant (Q2/D4), synchronous (D3), server-side registry resolution (D5), API-log skip (D6), utility ValueError → 400 (Q4/D7); `graphic_name` (ZPL `~DY`/`^XG` cache) wired through the `lib.stamp_document` re-export so it works over HTTP. Fresh-context review gate passed after remediation (P0 error/log-skip coverage added). Verified: 25 documents tests + 152 SDK tests green.
