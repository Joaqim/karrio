...
from .fixture import gateway, gateway_with_zpl
...
class TestPostNordShipment(unittest.TestCase):
    ...
    def test_create_shipment_zpl_url_from_config(self):
        # Connection-config label_type selects the ZPL endpoint too.
        with patch("karrio.mappers.postnord.proxy.lib.request") as mock:
            mock.return_value = "{}"
            karrio.Shipment.create(self.ShipmentRequest).from_(gateway_with_zpl)
            self.assertEqual(
                mock.call_args[1]["url"],
                f"{gateway.settings.server_url}/rest/shipment/v3/edi/labels/zpl?apikey=TEST_API_KEY",
            )

    def test_create_shipment_pdf_url_by_default(self):
        # No label_type anywhere: PDF endpoint, as before.
        with patch("karrio.mappers.postnord.proxy.lib.request") as mock:
            mock.return_value = "{}"
            karrio.Shipment.create(self.ShipmentRequest).from_(gateway)
            self.assertEqual(
